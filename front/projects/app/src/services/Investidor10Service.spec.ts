import { TestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { Investidor10Service } from './Investidor10Service';
import { API_CONFIG } from '../config/ApiConfig';
import { firstValueFrom } from 'rxjs';

describe('Investidor10Service', () => {
  let service: Investidor10Service;
  let httpMock: HttpTestingController;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [Investidor10Service],
    });

    service = TestBed.inject(Investidor10Service);
    httpMock = TestBed.inject(HttpTestingController);
  });

  afterEach(() => {
    httpMock.verify();
  });

  describe('getAcaoDetails', () => {
    it('deve buscar detalhes da ação usando código normalizado com espaços', () => {
      const responseMock = {
        codigo: 'VIVT3',
        empresa: 'VIVO - TELEFÔNICA BRASIL',
        dadosSobreEmpresa: [],
        informacoesSobreEmpresa: [],
        indicadoresFundamentalistas: [],
      };

      service.getAcaoDetails(' vivt3 ').subscribe(response => {
        expect(response).toEqual(responseMock);
      });

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/VIVT3`);
      expect(req.request.method).toBe('GET');
      req.flush(responseMock);
    });

    it('deve normalizar código lowercase para uppercase', () => {
      const responseMock = { codigo: 'VIVT3' } as any;

      service.getAcaoDetails('vivt3').subscribe(response => {
        expect(response).toEqual(responseMock);
      });

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/VIVT3`);
      expect(req.request.method).toBe('GET');
      req.flush(responseMock);
    });

    it('deve retornar erro padronizado quando API falhar', async () => {
      const apiErrorResponse = { message: 'Ticker inválido' };

      const promise = firstValueFrom(service.getAcaoDetails('XXXX'));

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/XXXX`);
      req.flush(apiErrorResponse, { status: 404, statusText: 'Not Found' });

      try {
        await promise;
        expect('não deveria chegar aqui').toBe(false);
      } catch (error: any) {
        expect(error.message).toBe('Ticker inválido');
        expect(error.status).toBe(404);
        expect(error.error).toEqual(apiErrorResponse);
      }
    });
  });

  describe('getProventos', () => {
    it('deve buscar proventos usando código normalizado', () => {
      const responseMock = {
        codigo: 'VIVT3',
        proventos: [{ data: '01/01/2024', valor: '1.50', tipo: 'DIVIDENDO' }],
      };

      service.getProventos('VIVT3').subscribe(response => {
        expect(response).toEqual(responseMock);
      });

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/VIVT3/proventos`);
      expect(req.request.method).toBe('GET');
      req.flush(responseMock);
    });

    it('deve normalizar código para uppercase', () => {
      const responseMock = { codigo: 'VIVT3', proventos: [] };

      service.getProventos('vivt3').subscribe(response => {
        expect(response).toEqual(responseMock);
      });

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/VIVT3/proventos`);
      expect(req.request.method).toBe('GET');
      req.flush(responseMock);
    });

    it('deve retornar erro padronizado quando API falhar', async () => {
      const apiErrorResponse = { message: 'Erro ao buscar proventos' };

      const promise = firstValueFrom(service.getProventos('VIVT3'));

      const req = httpMock.expectOne(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.investidor10}/VIVT3/proventos`);
      req.flush(apiErrorResponse, { status: 500, statusText: 'Server Error' });

      try {
        await promise;
        expect('não deveria chegar aqui').toBe(false);
      } catch (error: any) {
        expect(error.message).toBe('Erro ao buscar proventos');
        expect(error.status).toBe(500);
        expect(error.error).toEqual(apiErrorResponse);
      }
    });
  });
});
